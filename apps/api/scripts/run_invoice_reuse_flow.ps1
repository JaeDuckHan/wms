...
function Assert-Ok($res, $step) { if (-not $res.ok) { throw "[$step] ok=false" } }
...
try {
  ...
  $ib = Invoke-Api -Method POST -Url "$BaseUrl/inbound-orders" -Headers $authHeader -Body @{
    inbound_no="IB-IV-$ts"; client_id=$ClientId; warehouse_id=$WarehouseId; inbound_date=$today; status="draft"; memo="invoice-reuse inbound"; created_by=$UserId
  }
  ...
  $ibi = Invoke-Api -Method POST -Url "$BaseUrl/inbound-items" -Headers $authHeader -Body @{
    inbound_order_id=$created.inboundOrderId; product_id=$ProductId; lot_id=$LotId; qty=$InboundQty; invoice_price=10.0; currency="THB"; remark="invoice-reuse inbound item"
  }
  ...
  $ob = Invoke-Api -Method POST -Url "$BaseUrl/outbound-orders" -Headers $authHeader -Body @{
    outbound_no="OB-IV-$ts"; client_id=$ClientId; warehouse_id=$WarehouseId; order_date=$today; status="draft"; created_by=$UserId
  }
  ...
  $obi = Invoke-Api -Method POST -Url "$BaseUrl/outbound-items" -Headers $authHeader -Body @{
    outbound_order_id=$created.outboundOrderId; product_id=$ProductId; lot_id=$LotId; qty=$OutboundQty; box_type="BOX"; box_count=1; remark="invoice-reuse outbound item"
  }
  ...
  $first = Invoke-Api -Method POST -Url "$BaseUrl/invoices/issue" -Headers $authHeader -Body @{
    settlement_batch_id=$batchId; issue_date=$today; due_date=$dueDate; recipient_email="billing+$ts@example.com"; created_by=$UserId
  }
  Assert-Ok $first "invoice-first"
  $firstReused = [bool]$first.data.reused

  $second = Invoke-Api -Method POST -Url "$BaseUrl/invoices/issue" -Headers $authHeader -Body @{
    settlement_batch_id=$batchId; issue_date=$today; due_date=$dueDate; recipient_email="billing2+$ts@example.com"; created_by=$UserId
  }
  Assert-Ok $second "invoice-second"
  $secondReused = [bool]$second.data.reused

  if ($StrictFirstNew -and $firstReused) {
    throw "expected first invoice reused=false, got true"
  }
  if (-not $secondReused) {
    throw "expected second invoice reused=true, got false"
  }

  Write-Host "INVOICE_FIRST_REUSED=$firstReused"
  Write-Host "INVOICE_SECOND_REUSED=$secondReused"
