...
async function syncOutboundOrderBillingEvent(conn, outboundOrderId) {
  ...
  if (existing.length === 0) {
    const [inserted] = await conn.query(
      ...
      [order.client_id, order.warehouse_id, String(outboundOrderId), eventDate, totalQty]
    );
  ...
  await conn.query(
    `UPDATE billing_events
     ...
     WHERE id = ?`,
    [order.client_id, order.warehouse_id, eventDate, totalQty, existing[0].id]
  );
...
async function syncInboundOrderBillingEvent(conn, inboundOrderId) {
  ...
  if (existing.length === 0) {
    const [inserted] = await conn.query(
      ...
      [order.client_id, order.warehouse_id, String(inboundOrderId), eventDate, totalQty]
    );
    return inserted.insertId;
  }
