...
async function resolveKrwRate(conn, clientId, serviceCode, eventDate) {
  ...
  if (await hasTable(conn, "client_contract_rates")) {
    ...
    const currency = String(rateRows[0]?.currency || "KRW").toUpperCase();
    if (contractRate > 0 && currency === "KRW") {
      return contractRate;
    }
  }
  ...
  if (await hasTable(conn, "service_catalog")) {
    ...
    const defaultCurrency = String(serviceRows[0]?.default_currency || "KRW").toUpperCase();
    if (defaultRate > 0 && defaultCurrency === "KRW") {
      return defaultRate;
    }
  }
...
async function syncOutboundOrderBillingEvent(conn, outboundOrderId) {
  ...
  const unitPriceKrw = await resolveKrwRate(conn, order.client_id, "OUTBOUND_FEE", eventDate);
  const amountKrw = unitPriceKrw * totalQty;
  ...
  if (existing.length === 0) {
    const [inserted] = await conn.query(
      `INSERT INTO billing_events
       ...
       VALUES (?, ?, 'OUTBOUND_FEE', 'OUTBOUND', ?, ?, ?, 'KRW_FIXED', ?, ?)`,
      [order.client_id, order.warehouse_id, String(outboundOrderId), eventDate, totalQty, unitPriceKrw, amountKrw]
    );
  ...
  await conn.query(
    `UPDATE billing_events
     SET client_id = ?, warehouse_id = ?, event_date = ?, qty = ?, pricing_policy = 'KRW_FIXED',
         unit_price_krw = ?, amount_krw = ?, deleted_at = NULL
     WHERE id = ?`,
    [order.client_id, order.warehouse_id, eventDate, totalQty, unitPriceKrw, amountKrw, existing[0].id]
  );
...
async function syncInboundOrderBillingEvent(conn, inboundOrderId) {
  ...
  const unitPriceKrw = await resolveKrwRate(conn, order.client_id, "INBOUND_FEE", eventDate);
  const amountKrw = unitPriceKrw * totalQty;
  ...
  if (existing.length === 0) {
    const [inserted] = await conn.query(
      `INSERT INTO billing_events
        (client_id, warehouse_id, service_code, reference_type, reference_id, event_date, qty, pricing_policy, unit_price_krw, amount_krw)
       VALUES (?, ?, 'INBOUND_FEE', 'INBOUND', ?, ?, ?, 'KRW_FIXED', ?, ?)`,
      [order.client_id, order.warehouse_id, String(inboundOrderId), eventDate, totalQty, unitPriceKrw, amountKrw]
    );
    return inserted.insertId;
  }
